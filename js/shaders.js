...
export const fragmentShader = `
    ...
    void main() {
        ...
        float distortedFactor = uv.x - originalX;
        float parallaxDirection = -sign(0.5 - uMouse.x);

        vec2 parallaxOffset = vec2(
            parallaxDirection * abs(uMouse.x - 0.5) * uParallaxStrength * (1.0 + abs(distortedFactor) * uDistortionMultiplier),
            0.0
        );

        parallaxOffset.x *= edgeFactor;
